...
const Blog = ({ title, isHomepage }) => {
      ...
      try {
        ...
        if (user) {
          const likesResponse = await axios.get(
            `/api/user/getLikesByUser?id=${user.id}`
          );
  ...
  const handleBlogArticleHeartClick = (linkId) => {
    ...
    if (clickedBlogArticleIconId.includes(linkId)) {
      setClickedBlogArticleIconId(
        clickedBlogArticleIconId.filter((id) => id !== linkId)
      );
    } else {
    ...
    <>
      {loading ? (
        ...
        <section className="pb-60">
          {subCategory.length ? (
            <SubCategoryBreadcrumb subCategories={subCategory} title={title} />
          ) : (
            ""
          )}
                    ...
                    <div className="col-md-12" key={post.id}>
                      <div className="weekly-post-three">
                        ...
                        >
                          ...
                          <p>
                            {post.sub_title.length > 250
                              ? `${post.sub_title.slice(0, 250)}...`
                              : post.sub_title}
                          </p>
                              ...
                              <li className="col-3">
                                <span className="homeblog-link-icon-phone">
                                  <a
                                    href="#"
                                    onClick={() =>
                                      handleWhatsAppShare(post.seo_slug)
                                    }
                                  >
                                ...
                                <span className="homeblog-link-icon-facebook">
                                  <a
                                    href="#"
                                    onClick={() =>
                                      handleFacebookShare(post.seo_slug)
                                    }
                                  >
                                ...
                                <span className="homeblog-link-icon-twitter">
                                  <a
                                    href="#"
                                    onClick={() =>
                                      handleTwitterShare(post.seo_slug)
                                    }
                                  >
